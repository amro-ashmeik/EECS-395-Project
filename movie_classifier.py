...
class tweet_classifier():
	...
	def generate_report(self):
		...
		self.modelReport = 'NB'
		text_clf_NB = Pipeline([('tfidfv', TfidfVectorizer()), ('clf', MultinomialNB())])
		nested_score = cross_val_score(text_clf_NB, X=self.df.Script, y=self.df.Genre, cv=5, scoring=make_scorer(self.classification_report_with_accuracy_score))
		...
		text_clf_SVM = Pipeline([('tfidfv', TfidfVectorizer()), ('clf', SGDClassifier(loss='hinge', penalty='l2', alpha=1e-3, random_state=42, max_iter=5, tol=None))])
		nested_score = cross_val_score(text_clf_SVM, X=self.df.Script, y=self.df.Genre, cv=5, scoring=make_scorer(self.classification_report_with_accuracy_score))
	...
	# 	docs_train = self.df['Script'].tolist()
	# 	docs_train_label = self.df['Genre'].tolist()
	...
	# 		sorted_resultsdf.to_csv('idf_words.csv', encoding='utf-8')
